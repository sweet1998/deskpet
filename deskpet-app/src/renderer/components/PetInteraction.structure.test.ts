...
describe('PetInteraction unified conversation surface', () => {
  it('renders conversation history inside the input controls container', () => {
    ...
    expect(controlsSource).toContain('session-drawer')
    expect(controlsSource).toContain('filteredConversations')
    expect(controlsSource).toContain('createConversation')
    expect(controlsSource).toContain('deleteConversation')
    expect(controlsSource).toContain('exportConversation')
    ...
    expect(controlsSource).toContain('copyAnswer')
    expect(controlsSource).toContain('market-rows')
    expect(controlsSource).toContain('formatChange')
    expect(controlsSource).toContain('followup-context')
    expect(controlsSource).toContain('followup-target')
    expect(controlsSource).toContain('continueQuestion(message.id, message.text)')
