...
describe('PetInteraction unified conversation surface', () => {
  it('renders conversation history inside the input controls container', () => {
    ...
    expect(messageList.scriptSetup?.content).toContain('正在理解问题')
    expect(messageList.scriptSetup?.content).toContain('answerTextStarted')
    expect(messageList.scriptSetup?.content).toContain('streamingDisplayText')
    expect(messageList.scriptSetup?.content).toContain("message.type === 'text'")
    expect(messageList.scriptSetup?.content).toContain('!answerTextStarted.value')
    ...
    expect(drawerSource).toContain('暂无会话记录')
    ...
    expect(messageSource).toContain('copyMessage')
    expect(messageSource).toContain("message.role === 'user' ? '复制问题' : '复制回答'")
    expect(messageList.styles[0]?.content).toContain('.message.user .message-actions { position: absolute; right: calc(100% + 4px);')
