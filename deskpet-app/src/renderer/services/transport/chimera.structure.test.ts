...
describe('stock expert research routing', () => {
  it('uses server preparation and only creates thoughts for research requests', () => {
    ...
    expect(source).toContain("prepared.intent === 'answer_followup'")
    expect(source).toContain('结合最近对话直接解释上一条说法')
    expect(source).toContain("prepared.intent === 'role_capability'")
    expect(source).toContain('根据用户的具体问法自然回答')
    expect(source).toContain('没有当日快照时')
