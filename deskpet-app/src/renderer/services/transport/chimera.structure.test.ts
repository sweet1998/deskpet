...
describe('stock expert research routing', () => {
  it('uses server preparation and only creates thoughts for research requests', () => {
    ...
    expect(source).toContain('mergeContinuationText(options.continuationPrefix, result.text, streamedText)')
    ...
    expect(source).toContain('async function continueRoleText(requestId: string, roleId: RoleId)')
    expect(source).toContain('chat.getConversationTextHistory(requestId)')
    expect(source).toContain('chat.getAssistantText(requestId)')
    expect(source).toContain('preparedByRequest.get(requestId)')
    expect(source).toContain('await requestBackend(CONTINUATION_PROMPT')
    expect(source).toContain('continuation: true')
    expect(source).toContain('await requestDoubao(requestId, roleId, CONTINUATION_PROMPT')
