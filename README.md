...
- 数据：AKShare/腾讯提供盘中行情，Tushare Pro 可选提供专业历史数据，巨潮资讯提供官方公告，东方财富与新浪承担分项降级。
- 角色：麦麦、炒股专家。
...
问题缺少研究对象或存在多个有效候选时，股票专家会显示结构化澄清卡片。用户可以直接选择行情服务返回的候选项，也可以补充股票、板块、指数或分析条件；同一问题最多澄清两轮，仍无法确定时停止执行并提示重新描述。语义上属于新问题时会重置澄清轮次，不继承旧问题的限制。

股票专家的流程为：

```text
证券代码等确定格式由规则校验
  -> Qwen 3.7 Max 只读取当前问题并输出结构化研究计划
  -> 当前问题缺少目标或属于追问时，才进行第二次历史语义解析
  -> 后端校验目标来源、上下文关系、研究数据类型和时间范围
  -> Skill 选择白名单只读 MCP 工具
  -> 本地研究服务执行行情、新闻、筛选或板块扫描
  -> 计算技术与风险指标
  -> 校验工具结果是否匹配当前意图和目标类型
  -> 流式展示可核验的分析摘要
  -> 豆包生成正式回答
...
Qwen 3.7 Max 只判断问题类型、上下文关系、目标来源、所需研究数据和时间范围，不生成行情或确认证券代码，也不参与最终回答。历史目标只有在结构化关系为追问时才能继承；独立问题不会与上一轮主题一起送入首轮路由。分类请求关闭思考模式并采用低温度 JSON 输出；分类服务不可用时会明确报错，不会静默切回关键词路由。服务端通过 `DASHSCOPE_API_KEY` 或 `ROUTER_MODEL_API_KEY` 配置凭据，详细配置见 [`backend/README.md`](backend/README.md)。
