...
Qwen3 8B 只判断问题类型、上下文关系和原文中的标的名称，不生成行情或确认证券代码，也不参与最终回答。分类请求关闭思考模式并采用低温度 JSON 输出，5 秒内失败或置信度不足时自动回退原有规则，不阻断正常提问。服务端通过 `DASHSCOPE_API_KEY` 或 `ROUTER_MODEL_API_KEY` 配置凭据，详细配置见 [`backend/README.md`](backend/README.md)。
