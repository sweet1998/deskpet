...
Qwen3 8B 只判断问题类型、上下文关系、目标来源、所需研究数据和时间范围，不生成行情或确认证券代码，也不参与最终回答。历史目标只有在结构化关系为追问时才能继承；独立问题不会与上一轮主题一起送入首轮路由。分类请求关闭思考模式并采用低温度 JSON 输出，失败时回退受控规则，不阻断正常提问。服务端通过 `DASHSCOPE_API_KEY` 或 `ROUTER_MODEL_API_KEY` 配置凭据，详细配置见 [`backend/README.md`](backend/README.md)。
